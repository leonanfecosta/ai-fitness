...
def ia_decision(user: dict[str, LiteralString]) -> str:
    business_rules = """
    ...
    response = user_decision.send_message(
        f"Dados do usuário: {user}; Regras de negócio: {business_rules}"
    )
...
def user_details(
    user_id: str,
) -> tuple[Literal["Usuário não encontrado"], Literal[404]] | str:
    ...
    if request.method == "POST":
        ...
        return render_template(
            "user.html", user=user, user_id=user_id, message=ia_response
        )
